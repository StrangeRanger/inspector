...
def identifying_text(file): 
    for line in file:
        ...
        if fields[4].startswith("su["):
            ...
            # "Successful su for root by <username>"; identifies users who've successfully
            # became root using `su` and/or `su root`
            if fields[5] == "Successful" and conditions4:
                user = fields[-1]
                days[date]["+" + user] += 1  # A.2.
            # "FAILED su for root by <username>"; identifies users who've unsuccessfully
            # became root using `su` and/or `su root`
            elif fields[5] == "FAILED" and conditions4:
                user = fields[-1]
                days[date]["*" + user] += 1  # A.2.
            # "Successful su for <username> by <username>"; identifies users who've
            # successfully switched users using `su <username>`
